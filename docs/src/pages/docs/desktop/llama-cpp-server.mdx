...
### Intel Macs
- `llama.cpp-mac-amd64` (CPU only)
- `llama.cpp-macos-metal-x64` (native Metal for AMD/Intel/Nvidia GPUs)
- `llama.cpp-macos-vulkan-x64` (MoltenVK fallback for AMD GPUs)

<Callout type="info">
Apple Silicon automatically uses GPU acceleration through Metal. No additional setup required.
</Callout>

<Callout type="warning">
**Intel Macs with AMD GPUs**: Prefer the native Metal backend (`macos-metal-x64`) for stability and lower overhead. The Vulkan/MoltenVK backend remains available as a fallback but is still experimental.
</Callout>
