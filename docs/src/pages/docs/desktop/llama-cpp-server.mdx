...
- `llama.cpp-mac-vulkan` (AMD GPU acceleration via MoltenVK)
...
**Intel Macs with AMD GPUs**: The `mac-vulkan` backend enables GPU acceleration for Intel Macs with discrete AMD graphics cards (e.g., Radeon Pro 5500M). This uses MoltenVK to translate Vulkan calls to Metal. This feature is experimental.
</Callout>
